...
func (p *PredictionContext) ArrayEquals(o Collectable[*PredictionContext]) bool {
	...
	sameSize := len(p.returnStates) == len(other.returnStates) && len(p.parents) == len(other.parents)
	if !sameSize {
		return false
	}
	cacheEqual := p.cachedHash == other.cachedHash || p.cachedHash == 0 || other.cachedHash == 0
	if !cacheEqual {
